...
variable "namespace_rules_filter_action" {
  description = "Controls how SignalFx processes data from a AWS namespace. The available actions are one of Include or Exclude."
  default = "Include"
}

variable "namespace_rules_filter_source" {
  description = "Expression that selects the data that SignalFx should sync for the namespace associated with this sync rule. The expression uses the syntax defined for the SignalFlow filter() function; it can be any valid SignalFlow filter expression."
  default = "filter('code', '200')"
}

variable "namespace_rules_namespace" {
  description = "An AWS namespace having AWS metrics that you want to sync with SignalFx. See the AWS documentation on publishing metrics for more information."
  default = "AWS/EC2"
}
